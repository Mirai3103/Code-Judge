...
window.blazorMonaco.editor = {
  ...
  removeCircularReferences: function (orig) {
    if (Array.isArray(orig)) return orig.map(this.removeCircularReferences);
  ...
  colorizeElement: async function (elementId, options) {
    let promise = monaco.editor.colorizeElement(
  ...
  create: function (id, options, override, dotnetRef) {
    ...
    if (oldEditor !== null) {
      ...
      window.blazorMonaco.editors.splice(
        window.blazorMonaco.editors.findIndex((item) => item.id === id),
        1
      );
      oldEditor.dispose();
    }
    ...
    if (typeof monaco === "undefined")
      console.log(
        "WARNING : Please check that you have the script tag for editor.main.js in your index.html file"
      );
    ...
    window.blazorMonaco.editors.push({
      id: id,
      editor: editor,
      dotnetRef: dotnetRef,
    });
  ...
  createDiffEditor: function (
    ...
    if (oldEditor !== null) {
      ...
      window.blazorMonaco.editors.splice(
        window.blazorMonaco.editors.findIndex(
          (item) => item.id === id + "_original"
        ),
        1
      );
      window.blazorMonaco.editors.splice(
        window.blazorMonaco.editors.findIndex(
          (item) => item.id === id + "_modified"
        ),
        1
      );
      window.blazorMonaco.editors.splice(
    ...
    window.blazorMonaco.editors.push({
      ...
      dotnetRef: dotnetRefOriginal,
    });
    window.blazorMonaco.editors.push({
      id: id + "_modified",
      editor: editor.getModifiedEditor(),
      dotnetRef: dotnetRefModified,
    });

    if (oldModel !== null) editor.setModel(oldModel);
  },
  ...
  getEditorHolder: function (id, silent = false) {
    ...
    if (!editorHolder) {
      if (silent != false) {
        if (silent != true)
          // If silent is null, log a warning
          console.log(
            "WARNING : Couldn't find the editor with id: " +
              id +
              " editors.length: " +
              window.blazorMonaco.editors.length
          );
        return null;
      }
      throw (
        "Couldn't find the editor with id: " +
        id +
        " editors.length: " +
        window.blazorMonaco.editors.length
      );
      ...
      if (silent != false) {
        if (silent != true)
          // If silent is null, log a warning
          console.log(
            "WARNING : editor is null for editorHolder: " + editorHolder
          );
        return null;
      }
  ...
  addAction: function (id, actionDescriptor) {
    ...
    editorHolder.editor.addAction({
      ...
      run: function (editor, args) {
        editorHolder.dotnetRef.invokeMethodAsync(
          "ActionCallback",
          actionDescriptor.id
        );
      },
  ...
  executeEdits: function (id, source, edits, endCursorState) {
    ...
    if (endCursorState == "function") {
      endCursorState = (inverseEditOperations) => {
        return editorHolder.dotnetRef.invokeMethod(
          "ExecuteEditsCallback",
  ...
  revealLinesInCenterIfOutsideViewport: function (
    id,
    startLineNumber,
    endLineNumber,
    scrollType
  ...
  setInstanceDiffModel: function (id, model) {
    ...
    if (original_model == null || modified_model == null) return;
  ...
  model: {
    ...
    getLineDecorations: function (
      ...
      let model = this.getModel(uriStr);
      let result = model.getLineDecorations(
        lineNumber,
        ownerId,
        filterOutValidation
      );
    ...
    getLinesDecorations: function (
      ...
      let model = this.getModel(uriStr);
      let result = model.getLinesDecorations(
        startLineNumber,
        endLineNumber,
        ownerId,
        filterOutValidation
    ...
    getDecorationsInRange: function (
      uriStr,
      range,
      ownerId,
      filterOutValidation
      ...
      let model = this.getModel(uriStr);
      let result = model.getDecorationsInRange(
        range,
        ownerId,
        filterOutValidation
      );
    ...
    getOverviewRulerDecorations: function (
      uriStr,
      ownerId,
      filterOutValidation
    ) {
      let model = this.getModel(uriStr);
      let result = model.getOverviewRulerDecorations(
        ownerId,
        filterOutValidation
      );
      return blazorMonaco.editor.removeCircularReferences(result);
